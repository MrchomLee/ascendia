...
class Veredicto(BaseModel):
    id: int
    veredicto: Literal["aceptar", "rechazar", "dudosa"]
...
def exportar_revision(session: Session, *, manual_id: int, carpeta: Path | None = None) -> ExportacionRevision:
    ...
    grupos: dict[str, list[Question]] = {}
    for q in _revisables(session, manual_id):
        grupos.setdefault(q.window_key or f"{q.node_id}:sin-ventana", []).append(q)
    ventanas: dict[int, dict[str, Window]] = {}
    for key, preguntas in grupos.items():
        node_id = preguntas[0].node_id
    ...
    return ExportacionRevision(carpeta=revision, lotes=len(grupos), preguntas=sum(map(len, grupos.values())))
    ...
    for archivo in sorted(veredictos.glob("*.json")):
        ...
        for v in lote.veredictos:
            ...
            if q is None or q.manual_id != manual_id:
                ...
                continue
            if q.validation_status not in REVISABLES:
                resumen.omitidas.append((v.id, f"ya decidida ({q.validation_status})"))
                continue
            _aplicar(q, v, model_name, ahora)
...
def _revisables(session: Session, manual_id: int) -> list[Question]:
    preguntas = session.execute(
        select(Question)
        .where(Question.manual_id == manual_id)
        .where(Question.validation_status.in_(REVISABLES))
        .order_by(Question.id)
    ).scalars().all()
    return [q for q in preguntas if "revision" not in (q.metadata_json or {})]


def _aplicar(q: Question, v: Veredicto, model_name: str, ahora: datetime) -> None:
    """Los motivos del veredicto van en `revision`, aparte de los `motivos` de la
    revisión automática: así se ve quién dijo qué."""
    meta = dict(q.metadata_json or {})
    meta["revision"] = {
        "por": model_name, "veredicto": v.veredicto, "calificacion": v.calificacion,
        "motivos": list(v.motivos), "fecha": ahora.isoformat(),
    }
...
def _lote(key: str, breadcrumb: str, ventana: Window | None, preguntas: list[Question]) -> str:
    ...
    for q in preguntas:
        partes += [f"## Pregunta {q.id} · {q.question_type}", q.question_text]
...
```json
{{"veredictos": [
  {{"id": 123, "veredicto": "aceptar", "calificacion": 5, "motivos": []}},
  {{"id": 124, "veredicto": "rechazar", "calificacion": 1, "motivos": ["fuera de tema: pregunta un detalle del ejemplo"]}},
  {{"id": 125, "veredicto": "dudosa", "calificacion": 3, "motivos": ["depende de otra sección"]}}
]}}
```
